...
We assumed above that the exponents of $c$ and $d^\alpha$ are equal in the representation of $E$, and so are $C$ and $1/\pw$; it is clear that $\simulator$ can compute the correct $\sk$ even if they are different, since it knows $r_1$ and all the necessary exponents chosen by $\env$.
...
\begin{enumerate}
  ...
  \item Say $\env$ chooses $E = c$; then $E^{r_1} = c^{r_1}$. But Cramer--Shoup is obviously \emph{not} CCA-secure if the adversary additionally sees $c^{r_1}$. What saves us here is that $\env$ sees $E^{r_1}$ \emph{only at the end of $\user$'s session}. Indeed, the reduction $\red$ to the security of Cramer--Shoup roughly works as follows:
      \begin{enumerate}
        ...
        \item Finally, if the password guess in $\msg{2}^*$ is correct, $\red$ needs to simulate $\sk_U$. (This step is not needed in the game-based proof.)\footnote{We omit the remaining step which is to simulate $\msg{3}^*$ and $\server$'s session key $\sk_S$, as it is inconsequential to our main point.}
      \end{enumerate}
