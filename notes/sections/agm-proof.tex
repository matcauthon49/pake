...
\begin{lemma}
	Under the DDH assumption, we have $$\Pr[Z_5]=\Pr[Z_4] + \negl(\secpar).$$
\end{lemma}
\begin{proof}
	We show that any environment $\env$ which can differentiate between $\game_4$ and $\game_5$ can be used as a subroutine to build an adversary $\adv$ which can break the CCA-security game by successfully distinguishing between target ciphertexts. 
\end{proof}