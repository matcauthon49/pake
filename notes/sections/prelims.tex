...
\begin{equation*}
	\advddh_{\mathcal{G},\adv}(\lambda | \Gamma) = \left|\Pr[\adv(g^x,g^y,g^{xy}) = 1 : x,y\gets\Z_q^*] - \Pr[\adv(g^x,g^y,g^z) = 1 : x,y\gets\Z_q^*, z\gets\Z_q]\right|
\end{equation*}
and define the DSDH advantage of $\adv$ as
\begin{equation*}
	\advsdh_{\mathcal{G},\adv}(\lambda | \Gamma) = \left|\Pr[\adv(g^x,g^{x^2}) = 1 : x\gets\Z_q^*] - \Pr[\adv(g^x,g^y) = 1 : x,y\gets\Z_q^*]\right|
\end{equation*}
